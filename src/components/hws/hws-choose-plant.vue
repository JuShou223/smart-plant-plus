  ...
  <up-popup :show="show" mode="bottom">
    ...
    <view
      ...
      <view class="flex justify-between items-center mb-6">
        <text class="text-xl font-bold text-slate-900">选择植物</text>
      ...
      <scroll-view @scrolltolower="onPlantScrollToLower" scroll-y class="overflow-hidden space-y-3 no-scrollbar"
        style="height: 600rpx;">
        <template v-if="plants.length > 0">
          <view v-for="p in plants" :key="p.id" @click="handleSelectSpecies(p)"
            ...
            <image :src="p.plantImage" mode="aspectFill" class="w-14 h-14 rounded-xl bg-slate-100 shadow-sm" />
            <view class="flex-1 min-w-0">
              <text class="font-bold text-slate-800 text-sm block">{{ p.plantName }}</text>
...
// ==================== Lifecycle ====================
onMounted(() => {
  loadPlants(); // Ensure mock data is loaded
});
</script>
